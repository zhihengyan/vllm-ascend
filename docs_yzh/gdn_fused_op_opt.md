...
>
> **公式排版约定**（同 `gdn_impletiion.md`）：GitHub 要求行内公式的 `$` 左侧必须是 ASCII 空格，因此中文标点后会多出一个可见空格（如 `： $x$`）；另外公式内不使用 `\operatorname`、不写下划线、不依赖 `\,` `\{` 这类转义。改动公式时请保持。

---

## 0. 一句话结论

融合算子把 **算子数（~10 → 1）、外部布局转置、`h` 物化** 三件事一次性消掉了，但**总访存流量并没有真正下降**（复算约 41u vs 多算子路径 37.45u，同量级）；它把成本换到了另一处——**阶段 2（状态扫描）是一条只用到 `Nv` 个核的串行链**，其单核算术负载约为阶段 1 的 20 倍量级。
...
按 $(M,N,K)$ 计 MAC（ $N_v = 16$、`maxGroupLength = 5120`、40 核、 $T = 32\text{k}$）：

- **stage1 单核**：每任务 4 次 64 阶 matmul ≈ 2.1M MAC；任务数 $N_v \times 80 = 1280$，40 核 → 32 任务/核 → **≈ 67 MMAC/核**
- **stage2 单核**（只有 16 核活跃）：每 chunk 3 次 matmul（64×128×128）≈ 3.15M MAC；总 chunk 数 $T/c = 512$ → **≈ 1.61 GMAC/核**

比值约 **24 倍**。即：stage1/stage3 把 40 个核铺满，stage2 用 16 个核扛下 24 倍的活。**这是当前形态最该动的地方。**

---

## 4. 融合算子自身的优化路径（按预期收益排序）

排序依据：收益上限 × 是否落在关键路径 × 改动门槛。

| | 路径 | 命中什么 | 改动层 | 门槛 |
|---|---|---|---|---|
| **F1** | stage2 按 $D_v$ 切分 | 关键路径并行度 `Nv → Nv·k` | kernel | 中 |
| **F2** | 消掉核内预处理物化 | ~9u 流量 + 2 条 AIV 通路 | kernel | 中 |
| **F3** | matmul 流水化与跨架构对齐 | AIC 时间 15~30% | kernel | 中高 |
| **F4** | group 长度 $p$ 与 barrier 数扫描 | barrier 与 L2 压力的权衡 | tiling | 极低 |
| **F5** | 死 workspace 清理与常量收敛 | 1~3% + 前置保险 | tiling / kernel | 极低 |
...
**预期收益**：stage2 单核负载降 $\min(k, \text{aiCoreNum}/N_v)$ 倍。 $N_v=16$、40 核时 $k$ 取 2 就能从 16 核拉到 32 核（约 −40% 关键路径）；再往上要靠同样思路切 $D_k$（ $k$ 再 ×2，但 $D_k$ 切分需要完整 `vNew`，代价更高）。**若实测确认 stage2 是主瓶颈，这条应排第一。**
...
**预期收益**：F2a + F2b 合计约 **9u ≈ 总流量的 22%**，且完全是 kernel 内部改动，不动算子接口、不动 vLLM 侧。
...
**预期收益**：matmul 是 AIC 侧的全部工作量，流水化通常能拿 15~30% 的 AIC 时间；但**只在 AIC 是瓶颈时兑现**（F1 判断完成后才知道）。
...
**预期收益**：取决于实测的 barrier + 空转占比（用 §5 Step 2 的 timeline 读）。**改一个常量 + 扫参，建议第一批就做。**
